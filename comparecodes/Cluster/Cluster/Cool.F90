...
#include "Flash.h"
  ...
  real            :: eid_memory_reset, dt_local, eid_memory_new, epsilon
  integer         :: n_new, nn_new, i_loop
...
!! ********************* SUBCYCLING IF COOLING IS FAST **********************************
!              eid_memory_reset = eid
!              n_new   = 1
!              dt_local = dt
!              epsilon = 1.0e-6 !0.25
!
!10            continue ! resetting (i.e., starting this code timestep from scratch)
!              eid            = eid_memory_reset
!              eid_memory_new = eid_memory_reset
!              nn_new = n_new
!
!
!!              nn_new   = 1000
...
!
!!                call cool_integrate(eid, time, dt)
!                 call cool_integrate(eid, time, dt_local)
!                 if (abs(eid_memory_new-eid)/abs(eid_memory_new) >= epsilon) then
!                    dt_local = dt_local/2.0
!                    n_new = n_new*2
!                    goto 10
!                 endif
!                 eid_memory_new = eid
