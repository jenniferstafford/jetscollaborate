...
#include "UHD.h"
  ...
  logical, save :: hy_useBiermann3T = .false.
  logical, save :: hy_biermannSource = .false.
  logical, save :: hy_useHall = .false.
#endif
           ...
           do i=i0-2,imax+2
              ...
              if (hy_useViscosity) then
                 ...
                 call Viscosity(U(TEMP_VAR,i,j,k),U(DENS_VAR,i,j,k),&
                                U(SPECIES_BEGIN:SPECIES_END,i,j,k), &
                                viscDynamic(i,j,k),viscKinematicUnused)
              endif

              if (hy_useConductivity .and. hy_addThermalFlux) then
                 !! Get heat conductivity
                 call Conductivity(U(:,i,j,k),cond(i,j,k),component=2)
              endif
#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
           ...
           if (hy_useDiffuse) then
              ...
              if (hy_useConductivity .and. hy_addThermalFlux) then
                 call hy_uhd_addThermalFluxes&
                      xflux(blockID,blkLimitsGC,i,j,k,xflux(F01DENS_FLUX:F01DENS_FLUX+HY_VARINUM-1,i,j,k),cond,DIR_X,Dcffx(i,j,k))
              endif
...
#ifdef FLASH_USM_MHD           
           if ((hy_useBiermann .and. hy_useBiermann3T) .and. (.not. hy_biermannSource)) then
              call hy_uhd_addBiermannFluxes &
                   (blockID,blkLimitsGC,i,j,k,xflux(F01DENS_FLUX:,i,j,k), DIR_X)
           endif 
#endif

#ifdef FLASH_USM_MHD           
           if (hy_useHall) then
              call hy_uhd_addHallFluxes &
...
!#endif
           ...
           if (hy_useDiffuse) then
              ...
              if (hy_useConductivity .and. hy_addThermalFlux) then
                 call hy_uhd_addThermalFluxes&
                      yflux(F01DENS_FLUX:F01DENS_FLUX+HY_VARINUM-1,i,j,k),cond,DIR_Y,Dcffy(i,j,k))
              endif
           ...
           if ((hy_useBiermann .and. hy_useBiermann3T) .and. (.not. hy_biermannSource)) then
              call hy_uhd_addBiermannFluxes &
                   (blockID,blkLimitsGC,i,j,k,yflux(F01DENS_FLUX:,i,j,k), DIR_Y)
           endif            
#endif
...
!#ifdef COMPUTE_DT_FLUX
           if (hy_hydroComputeDtOption == 1) then
              !! Call for dt calculation
              if (hy_geometry == CARTESIAN .OR. hy_geometry == POLAR) then 
                 ! the 'z' coordinate is not angular in Cartesian and polar coords
                 dz = del(DIR_Z)
              elseif (hy_geometry == CYLINDRICAL) then
                 dz = xCenter(i)*del(DIR_Z)
              else ! Angular coordinates in 3D: Spherical
...
!#endif
           ...
           if (hy_useDiffuse) then
              ...
              if (hy_useConductivity .and. hy_addThermalFlux) then
                 call hy_uhd_addThermalFluxes&
                      (blockID,blkLimitsGC,i,j,k,zflux(F01DENS_FLUX:F01DENS_FLUX+HY_VARINUM-1,i,j,k),cond,DIR_Z,Dcffz(i,j,k))
              endif
           ...
           if ((hy_useBiermann .and. hy_useBiermann3T) .and. (.not. hy_biermannSource)) then
              call hy_uhd_addBiermannFluxes &
                   (blockID,blkLimitsGC,i,j,k,zflux(F01DENS_FLUX:,i,j,k), DIR_Z)
           endif            
#endif
